...
import { CreateUserModalProps, DocumentPermission, UserFormData } from '../types/types';
...
const CreateUserModal: React.FC<CreateUserModalProps> = ({
  ...
  const validateStepOne = () => {
    ...
    if (!formData.password) return 'La contrasena es obligatoria.';
    ...
    if (currentStep === 1) {
      ...
      if (formData.rol === 'admin') {
        try {
          ...
          setError(err?.message || 'No pudimos crear el usuario.');
        }
        return;
      }
              ...
              <div className="flex items-start justify-between gap-4 px-4 py-4 sm:px-6">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#1e2b66]">Nuevo acceso</p>
                    <h2 id="create-user-title" className="mt-1 text-xl font-semibold tracking-normal text-slate-950 sm:text-2xl">
                      Crear usuario
                    </h2>
                    <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">
                      Registra la cuenta y define a que documentos puede acceder.
                    </p>
                  </div>
                </div>
                <button
                      ...
                      >
                        <div className="mb-5 flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-slate-600">Los datos marcados son necesarios para activar la cuenta.</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          ...
                          <div>
                            <label className={labelClassName}>Contrasena <span className="text-red-600">*</span></label>
                            <div className="relative">
                              <KeyRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                              <input type="password" name="password" value={formData.password} onChange={handleChange} className={`${fieldClassName} pl-10`} placeholder="Minimo 6 caracteres" disabled={isLoading} />
                            </div>
                          </div>
                          <div>
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                          <div className="mb-4 flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="mt-1 text-sm leading-6 text-slate-600">Activa los documentos y capacidades necesarias para este usuario.</p>
                            </div>
                          </div>
              ...
              <footer className="shrink-0 border-t border-slate-200 bg-white px-4 py-3 sm:px-6 sm:py-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-slate-600">
                    {currentStep === 1 ? 'Completa los datos base del usuario.' : `${stats.withAccess} documento${stats.withAccess !== 1 ? 's' : ''} con acceso.`}
                  </p>
                  <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    ...
                    >
                      ...
                      ) : currentStep === 1 && formData.rol !== 'admin' ? (
                        <>
                          Siguiente
